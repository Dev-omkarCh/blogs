import { ArrowUpRight, Clock, Edit3, Eye, Globe, Heart, LayoutGrid, List, Plus, Search, Trash2 } from 'lucide-react'
import { useState } from 'react'
import { useNavigate } from 'react-router-dom';

const MyBlogs = ({ blogs } : { blogs: any[] }) => {
        ...
        {viewMode === 'grid' ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredBlogs.map((blog) => (
              <div 
                key={blog._id} 
                className="group flex flex-col bg-slate-900/20 border border-slate-900 rounded-[2.5rem] 
                overflow-hidden hover:border-indigo-500/50 transition-all duration-500"
              ...
              >
                ...
                <div className="p-8 flex flex-col flex-1">
                  <h3 className="text-xl font-black text-white tracking-tight mb-4 line-clamp-2 min-h-14">{blog.title}</h3>
          ...
          <div className="space-y-4">
            {filteredBlogs.map((blog) => (
              <div key={blog._id} className="group flex items-center gap-6 bg-slate-900/20 border border-slate-900 p-4 rounded-3xl hover:bg-white/2 transition-all">
